...
//имя тв seocategory
    ...
    public function getFilterValues($content_ids, $filter_tv_ids = '')
    {
        ...
        if ($content_ids != '') {//берем только если есть какие-то документы
            $sql = "SELECT tmplvarid,contentid,value FROM " . $this->modx->getFullTableName('site_tmplvar_contentvalues') . " WHERE contentid IN (" . $content_ids . ") " . ($filter_tv_ids != '' ? " AND tmplvarid IN (" . $filter_tv_ids . ")" : "");
            ...
            $tvs_default = $this->modx->db->query('SELECT id,name,default_text as dt FROM ' . $this->modx->getFullTableName('site_tmplvars') . '  where id in (' . $filter_tv_ids . ') and default_text <> ""');
            $c_ids = explode(',', $content_ids);

            while ($tvd = $this->modx->db->getRow($tvs_default)) {

                foreach ($c_ids as $c_id) {
                    if (isset($filter_values[$tvd['id']][$tvd['dt']]['count'])) {
                        $filter_values[$tvd['id']][$tvd['dt']]['count'] += 1;
                    } else {
                        $filter_values[$tvd['id']][$tvd['dt']]['count'] = 1;
                    }
                }
        ...
        if (!empty($curr_filter_values)) {//берем только если есть какие-то документы
            foreach ($curr_filter_values as $tv_id => $v) {
                if (isset($v['content_ids']) && $v['content_ids'] != '') {
                    ...
                    $sql = "SELECT tmplvarid,contentid,value FROM " . $this->modx->getFullTableName('site_tmplvar_contentvalues') . " WHERE contentid IN (" . $content_ids . ") " . ($filter_tv_ids != '' ? " AND tmplvarid ={$tv_id}" : "");
                    $q = $this->modx->db->query($sql);
                    $content_values = array();
                    while ($row = $this->modx->db->getRow($q)) {
                        $content_values[] = $row['contentid'];
                    ...
                    $tvs_default = $this->modx->db->query('SELECT id,default_text as dt FROM ' . $this->modx->getFullTableName('site_tmplvars') . '  where id in (' . $tv_id . ') and default_text <> ""');
                    $c_ids = explode(',', $content_ids);
                    while ($tvd = $this->modx->db->getRow($tvs_default)) {

...
$tv_list = array();
$sql = "SELECT a.`id`,a.`name`,a.`caption` FROM " . $modx->getFullTableName('site_tmplvars') . " as a, " . $modx->getFullTableName('site_tmplvar_templates') . " as b WHERE a.`category` IN (" . $param_cat_id . ") AND `a`.`id` = `b`.`tmplvarid` AND `b`.`templateid` = " . $params['product_template_id'] . "  ORDER BY b.`rank` ASC, a.`caption` ASC";

...
в стандартном режиме устанавливает следующие плейсхолдеры:<br><br>
...
**eFilterTime** - технический, время отработки фильтрации<br><br>

В скобках - значение по-умолчанию.<br><br>
...
### логика шаблонизации вывода формы фильтра сниппета iFilter
**cfg** - (default). Папка с шаблонами для вывода элементов формы фильтрации (относительно assets/snippets/eFilter/tpl/)<br><br>
**tpls** - ([]). Массив шаблонов для нужных ТВ [ 'brand' => 'new' ] пытается найти сначала шаблон {type}RowNew/{type}OwnerNew (camelCase, например checkboxesOwnerNew/checkboxesOwnerRow).<br>
Если таких шаблонов в папке cfg нет происходит поиск шаблона по имени ТВ - {type}RowBrand/{type}OwnerBrand (camelCase). Если и он не найден - используются стандартные<br><br>
**prepare** - функция модификации данных перед выводом function prepareFilter($fields, $type, $params), возвращает массив $fields.<br>
$fields - массив полей, можно модифицировать текущие (для сильных духом) либо подкинуть какие-то свои дополнительные для использования в шаблоне<br>
$type - тип шаблона row/owner (элемент либо обертка)<br>
$params => [ 'filter' => 'массив данных о текущем фильтре, имя тв, подпись, список значений', 'types' => 'массив типов используемых фильтров', 'config' = 'массив из конфига данного фильтра'] - данные для более тонкой настройки prepare<br>
...
### дополнительные параметры вызова сниппета eFilter
        ...
        &autoSubmitDelay=`1000` - задержка ms сабмит формы (default - 100)

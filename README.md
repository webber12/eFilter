...
### дополнительные параметры вызова сниппета eFilter
        ...
        &submitDocPage=`1` - сабмит формы будет произведен на страницу, заданную в docid<br>
        &allowZero=`1` - разрешает 0 в значениях чекбокса<br>
        &useRegexp=`1` - только для сильных духом:)<br>
        &commaAsSeparator=`all`||&commaAsSeparator=`1,2,3` - распознавать запятую как разделитель значений для всех TV (all) либо только для указанных по id (1,2,3)<br>
<br>

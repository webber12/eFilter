...
Если таких шаблонов в папке cfg нет происходит поиск шаблона по имени ТВ - {type}RowBrand/{type}OwnerBrand (camelCase). Если и он не найден - используются стандартные
...
$type - тип шаблона row/owner (элемент либо обертка)
$params => [ 'filter' => 'массив данных о текущем фильтре, имя тв, подпись, список значений', 'types' => 'массив типов используемых фильтров', 'config' = 'массив из конфига данного фильтра'] - данные для более тонкой настройки prepare
